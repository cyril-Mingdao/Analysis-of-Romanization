...
/* v24 換用 pic4.png 當母片（規則牌改為 10 條）。pic4 的圖面相對 pic3 有一層仿射差
 * （以箭頭區整塊灰階做最小平方對位：x' = 0.995055x − 0.017、y' = 1.010568y − 5.571，
 *  殘差 RMS 由 45.5 降到 8.0 灰階），越靠右偏移越大——最右側「8 ↕ 4」約差 3.1px，
 * 沿用 pic3 座標時紅色遮罩會蓋不住母片黑箭頭、上緣露出黑邊。
 * 下列座標即為套用該仿射後、對齊 pic4 的值。
 * v17 原註：pic3 相對更早的 master-bg 亦有 x' = 1.002019x − 0.017、y' = 1.000936y − 0.572。*/
const MASTER_ARROWS = [
  { tail: [385.76, 417.65], tip: [233.17, 417.50], heads: 'tip'  }, // 2 → 1
  { tail: [595.77, 417.94], tip: [443.22, 418.27], heads: 'tip'  }, // 4 → 2
  { tail: [234.76, 541.70], tip: [387.25, 541.48], heads: 'tip'  }, // 7 → 3
  { tail: [595.74, 542.92], tip: [443.19, 542.81], heads: 'tip'  }, // 8 → 3
  { tail: [209.87, 442.27], tip: [209.77, 510.90], heads: 'tip'  }, // 1 → 7
  { tail: [412.16, 508.25], tip: [412.38, 439.88], heads: 'tip'  }, // 3 → 2
  { tail: [624.42, 518.47], tip: [624.47, 449.87], heads: 'both' }, // 8 ↕ 4（母片為雙箭頭）
  { tail: [282.64, 473.97], tip: [239.44, 517.62], heads: 'tip'  }, // 5 ↘ 7
];
